(function (win, $) {
  ...
  var highlightPuzzleInput = function (e) {
    var focusedPuzzleCount = Math.max.apply(
      null,
      $(e.target).closest('.puzzle-cell').data('puzzle')
    );
  ...
  var drawArrayMap = function () {
    ...
    for (i = 0; i < across.length; i++) {
      for (j = 0; j < across[i].length; j++) {
        Crossword.map[across[i].y - 1][across[i].x + j - 1].push(
          across[i].number
        );
      }
    ...
    for (i = 0; i < down.length; i++) {
      for (j = 0; j < down[i].length; j++) {
        Crossword.map[down[i].y + j - 1][down[i].x - 1].push(down[i].number);
      }
  ...
  var renderPuzzle = function () {
    ...
    for (i = 0; i < Crossword.map.length; i++) {
      $('<div>')
        .appendTo($puzzleParent.find('.crossword-puzzle'))
      ...
      for (j = 0; j < Crossword.map[i].length; j++) {
        ...
        // Append puzzle cell.
        $('<div>')
          .appendTo($puzzleParent.find('.puzzle-row-' + (i + 1)))
        ...
        if (
          Crossword.map[i][j].length > 0 &&
          puzzleCount < Math.max.apply(null, Crossword.map[i][j])
        ) {
          $puzzleParent
            .find('.puzzle-cell-' + (j + 1) + (i + 1))
            .append('<span>' + (puzzleCount + 1) + '</span>');
          puzzleCount = Math.max.apply(null, Crossword.map[i][j]);
        }
      }
    }
  ...
  Crossword.resizePuzzle = function () {
    $puzzleParent.find('.puzzle-cell').css({
  ...
  var renderClues = function (direction, serialNum, description, length) {
    ...
    if ($clueParent.find('h2.crossword-label-' + direction).length === 0) {
      ...
      $('<ul>')
        .appendTo($clueParent)
        .addClass('crossword-clues-' + direction);
    }
    $('<li>')
      .appendTo($clueParent.find('ul.crossword-clues-' + direction))
